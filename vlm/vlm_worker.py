...
async def main_async():
    ...
    parser.add_argument("--vlm_model", default="gemma3:12b",
                        help="Ollama vision model tag. Runs locally via Ollama — pull it first "
                             ...
                             "Gemini's free tier caps out at 5 requests/minute). Three local "
                             ...
                             "new inference engine dropped mllama support with no fix/ETA — and "
                             "qwen2.5vl:7b loaded fine but was both slow and weak on this "
                             "project's GPU (see Pascal note below), so the default moved to "
                             "gemma3:12b, which is stronger at schema-constrained JSON and the "
                             "hardware's per-frame cost is tolerable given --every 60. Ollama's "
                             "new engine only natively supports Llama 4, Gemma 3, Qwen 2.5 VL, "
                             "and Mistral Small 3.1 — pick a replacement from that set. If "
                             "gemma3:12b is too slow, gemma3:4b is the lighter fallback; "
                             "qwen2.5vl:7b still works if you want to try it again. Do NOT use "
